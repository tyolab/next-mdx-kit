...
| `@tyolab/next-mdx-kit/build` | `buildPost` (serialize a raw MDX string) — **fs-free**; import from here (not `.`) in webpack/static-export apps so the fs-based loaders aren't dragged into the bundle |
